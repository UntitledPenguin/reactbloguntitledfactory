...
- **目标定位（Object localization）** 回答： 👉 “图像中的物体在哪里？”  它不仅要判断是什么，还框出它的位置。比如下面我们就圈出了狗狗，自行车和车。![[yoloalgorithm.jpg]]
- 而**语义分割（Semantic segregation）** 则更进一步地回答👉 “这个像素是什么”。它将图片的每一个像素都分入不同的分类，从而获取图片的每个部分是什么，如下图所示，图片就按照不同类别：交通牌，行人，地面，车行道，车，天空，树木，房屋被划分清楚了。 ![[carseg.png]]
