...
不同于传统的通过代数方法求解拟合曲线的方法，梯度下降法也可以帮助我们通过迭代寻找局部最小值。
...
而式中分母上的2纯纯是为了求导后便于约分添加的。为了实现梯度下降，对此式向w和b分别求偏导，可以得到下面两式 ：
...
可以看到数据本身呈现喇叭状，很显然房价和面积之间的关系呈现异质性，不是一个简单的系数。毕竟影响房屋面积和价格的变量有很多，而我们这次进行单一变量的回归具有很大的局限性。

![Dataset](/static/images/tech/0608/output_15_1.png)

所以在本次小实验前我们对数据进行了一个小筛选，仅选择公寓“Flat/Apartment”和伦敦西南地区的房屋（Postal code前两位是SW）的房屋参与回归。这样可以一定程度上控制其他相关变量的干扰，使得用线性回归的结果更准确一些（尽可能的）。
...
| --------------------------- | ----------------------------------- | ---------------- | ------------------------------------------------ | -------------------------- |
| 01. Cubic-Square-SquareRoot | $y=w_1x+w_2x^2+w_3x^3+w_4\sqrt x+b$ | 0.1551092909     | (0.43183551,0.72930492,-0.20345624 ,-0.14941295) | 4.3262670349445654e-16     |
| 02. Square                  | $y=w_1x+w_2x^2+b$                   | 0.1550745809     | (0.36864666,0.481259)                            | 8.56581257254045e-17       |
| 03. Cubic-Square            | $y=w_1x+w_2x^2+w_3x^3+b$            | **0.1548145757** | (0.30049734,0.67763631,-0.15096104)              | 9.715853794897574e-17      |
| 04. Square-Squareroot       | $y=w_1x+w_2x^2+w_4\sqrt x+b$        | 0.1552877450     | (0.4223379 ,0.48141813, -0.05834984)             | 2.2278009840078895e-16     |
| 05. SquareRoot              | $y=w_1x+w_4\sqrt x+b$               | 0.1649958817     | (1.45124432, -0.66711398)                        | 1.6636835439198916e-15<br> |
